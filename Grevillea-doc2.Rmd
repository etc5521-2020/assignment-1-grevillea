...
left_join(terrain_non_finishes, total_stage_riders, by = "year") %>%
  filter(year >= "1969") %>%
  select(year, Type_terrain, non_finish, riders) %>%
  mutate(pct_not_finish = (non_finish/riders)*100) %>%
  drop_na() %>%
  ggplot(aes(x = year,
             y = pct_not_finish)) +
  geom_point() +
...
The body mass index (BMI) is a measurement of a persons weight relative to their height and effectively indicating the level of body fat of an individual. BMI is widely used as an indicator of health and fitness. The BMI of the winnings riders was calculated using by dividing their weight over height^2. The plot was originally 
...
ggplot(aes(x = start_date, 
  ...
  theme_linedraw() +
  theme_bw() +
  ggtitle("Winning riders BMI") +
  xlab("Year") +
  ylab("BMI") + 
  labs(color = "Top Teams")
```

Figure \@ref(fig:bmi) displays the BMI of each winning rider with
