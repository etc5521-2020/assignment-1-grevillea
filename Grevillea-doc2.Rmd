...
```{r stage-clean-rider-name}
stage_clean %>%
  select(rider) %>%
  mutate(name = strsplit(rider, split = "^[A-Za-z]+[[:space:]]+", "", b")))
...
stage_clean %>%
  ...
  filter(rank %in% c("DNS", "OTL")) %>%
  group_by(stage_results_id) %>%
  summarise(not_finished = n()) %>%
  mutate(stage_results_id = as.factor(stage_results_id)) %>% #sepereate and paste stage- to go behind the number
  arrange(stage_results_id) %>%
  ggplot(aes(x = stage_results_id,
             y = not_finished)) +
  geom_col(stat = "identity") +
  theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5))
```
