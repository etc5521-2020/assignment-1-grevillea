...
Most winners are ranked below 50 for most of the stages except for the last stage. Moreover, it can be observed that in the last few stages, particularly 17-20, overall winners pick up their pace and achieve top 10 ranks. Furthermore, this also suggests that overall winners kept a consistent pace throughout the race, where riders who exerted more energy in earlier stages are unable to keep up with the overall winners. 
...
```{r}
ggplot(stage_finish_wide) +
  geom_line(aes(x = year,
                y = total_riders_first_stage),
            colour = "blue") +
  geom_line(aes(x = year,
                y = total_riders_last_stage),
            colour = "red") 
```

```{r finish-rate}
stage_finish_wide <- stage_finish_wide %>%
...
test <- stage_data %>%
  group_by(year, stage) %>%
  tally() %>%
  summarise(first_stage = min(n),
            last_stage = max(n)) %>%
  pivot_longer(cols = first_stage:last_stage,
               names_to = "first_last_stage",
               values_to = "total_riders")

stage_total_riders <- stage_data %>%
  group_by(year, stage) %>%
  tally()

stage_total_riders<- stage_total_riders %>%
  inner_join(test, 
             by = c("year", "n" = "total_riders"))
