...
tdf_stages_2010s <- tdf_stages %>%
  filter(between(year, 2010, 2019)) %>%
  rename(stage_winner = winner,
         stage_winner_country = winner_country)

# Change "P" to stage 0
## Represents initial time trial of the race
tdf_stages_2010s$stage <- ifelse(tdf_stages_2010s$stage == "P", "0", as.factor(tdf_stages_2010s$stage)) 

tdf_winners <- tdf_winners %>%
  mutate(year = year(start_date))

# Extract overall winners' names
winners_2010s <- tdf_winners %>%
  filter(between(year, 2010, 2019)) %>%
  select(winner_name, year)

# Change order of winner names to match with stage_data rider names
winners_2010s$winner_name <-
  str_replace(string = winners_2010s$winner_name, pattern = "([^ ]+) ([^ ]+)", replacement = "\\2 \\1")

# Extract stages details 
stage_winners_2010s <- stage_data %>%
  filter(between(year, 2010, 2019)) %>%
  # include stage data of overall winner only
  inner_join(winners_2010s, by = c("year", "rider" = "winner_name")) %>% 
  left_join(tdf_stages_2010s, by = c("year", "stage")) %>% # Add stage details
  mutate(rank = as.numeric(rank)) # Change rank to numeric for plotting

```

```{r}
ggplot(stage_winners_2010s) +
...
At which stage, are they ranked the best? What kind of terrain is it?

next plot: https://twitter.com/zahlooh/status/1249713409584771072
- group by terrain, see which terrains overall winners are good/bad at.

next plot: https://twitter.com/delaBJL/status/1247552025560338432
- chicklet plot
- How many of each terrains in a race?
