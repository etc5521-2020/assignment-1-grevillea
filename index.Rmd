...
```{r, fig.cap="Visualise the missing value in stage data", fig.align= "center"}
stage_clean %>%
...
- The `tdf_winner` data comes from _tdf_winners.csv_. The data contains information about 106 winners of the Tour de France from 1903 to 2019. The variables are showing in the Table \@ref(tab:winners-table).  

```{r winners-table, echo=FALSE}
winners_info <- tibble(Variable = c("edition", "start_date", "winner_name", "winner_team", "distance", "time_overall", "time_margin", "stage_wins", "stage_led", "height", "weight", "age", "born", "died", "full_name", "nickname", "birth_town", "birth_country", "nationality"),
                          Class = c("integer", "double", "character", "character", "double", "double", "double", "double", "double", "double", "double", "integer", "double", "double", "character", "character", "character", "character", "character"),
                          Description = c("Edition of the Tour de France", "Start date of the Tour", 
                                          ...
                                          "Age as winner",
                                          "year born", 
                                          "Year died", "Full name", "Nickname", "Birth town", "Birth country", "Nationality")) 
...
stage_data_info <- tibble(Variable = c("edition", "year", "stage_results_id", "rank", "time", "rider", "age", "team", "points", "elapsed", "bib_number"),
                          Class = c("integer", "double", "character", "character", "double", "character", "integer", "character", "integer", "double", "integer"),
...
### How does the distance and speed of the Tour de France change? {-#mysection}
...
#legendtitle <- list(yref='paper',xref="paper",y=1.05,x=1.1, text="Mountain and hilly stage(%)",showarrow=F)

fig_dist <- dist_data %>%
  highlight_key(~perc_mount_hill_cat) %>%
  plot_ly(
    x = ~year, y = ~distance, text = ~perc_mount_hill_cat, color = ~perc_mount_hill_cat, mode = "markers", 
    textposition = "none", hoverinfo = "x+y"
  ) %>%
  add_annotations( text="% of mount & hill", xref="paper", yref="paper",
                  x=1.02, xanchor="left",
                  y=0.8, yanchor="bottom",    # Same y as legend below
                  legendtitle=TRUE, showarrow=FALSE ) %>%
  layout(legend=list(y=0.8, yanchor="top" ),
         title = "Tour de France distance from 1903-2019",
...
ggplot(filter(dist_data, !is.na(speed), !is.na(bmi))) +
