...
winners %>%
  ...
  ggplot(aes(x = decade, y = time_overall)) +
  geom_line(colour="steelblue") +
  ylab("Average Hours") +
  xlab("Year") +
...
```{r, fig.asp="Average difference in finishing time between the race winner and the runner up"}
winners %>%
  group_by(decade = 10 * (year(start_date) %/% 10)) %>%
  summarise(time_margin = mean(time_margin, na.rm = TRUE)) %>%
  ggplot(aes(x = decade, y = time_margin * 60)) +
  geom_line(colour="steelblue") +
