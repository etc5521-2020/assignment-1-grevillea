...
```{r finish-hour, fig.cap="Average hours taken by the winner to complete the race"}
...
As showing in Figure \@ref(fig:finish-hour), the average of total hours taken by the winner to complete the race are increased before 1920 and reached its peak around 1920, which is over two hundred hours. After 1920, the average total hours began to decline, and until the past decades, the average total hours have dropped to less than ninety hours. However, this result is still not enough to support that the riders are getting faster than before and the competition getting more competitive, because the reduction in average total hours is also affected by the distance of the races.

```{r time-margin, fig.cap="Average difference in finishing time between the race winner and the runner up"}
