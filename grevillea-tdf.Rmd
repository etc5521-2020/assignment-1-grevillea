...
tdf_stages %>%
  # Replace capitalised words with lower-case letters
  clean_names %>%
  write_csv(here::here("data/tdf_stages.csv")) 
```
...
# 2. Is there a change in physcial/biological traits of overall winners over the years?

The Tour de France conists of a range of stage types which across differnt terrains which range from flat groud, mountains and hilly. Each of this terrains may suit a certain type of body compostion given the different elements and requirements for the specific terrain such as flat terrain and time trials better suits sprinters who are more likely to have a more muscular frame to generate power and mountain stages better suiting a slimmer build given how strenuous and endurance based those stages are. Given the overall winner is required to have the lowest elapsed time, this section will be assessing how the winners physical and biological traits match up with the other competitors and stage specialists.

## Do overall winners, top sprinters or climbers have similar features (e.g. body type, age)? 

The top climbers, sprinters and time trial lists were isolated by targetting the winners of the flat and mountain stages in the rdf_stages section, with the top 6 riders from each category being selected. However, their height and weight were not included in the original dataset and in order to compare the body types with the overall winner their dimensions were sourced online and compiled into a tibble and then row binded together.
